...
# =========================
BOT_TOKEN = ""
PANEL_URL = "https://gp.ligitnodes.xyz"  # no trailing slash
PANEL_API_KEY = ""
API_KEY = ""
APP_API_KEY = ""
ADMIN_IDS = "1405866008127864852"
PANEL_NODE_ID = "1"  # node id to select allocations
...
class ManageServerView(discord.ui.View):
    def __init__(self, token: str, serverid: str):
        ...
        self.base = f"https://gp.ligitnodes.xyz/api/client/servers/{self.serverid}"
...
async def manage(ctx, token: str = None):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get("https://gp.ligitnodes.xyz/api/client", headers=headers) as resp:
            if resp.status != 200:
...
async def get_server_internal_id(identifier):
    url = "https://gp.ligitnodes.xyz/api/application/servers"
...
async def create_ad(ctx, email: str, password: str, is_admin: str):
    ...
    url = "https://gp.ligitnodes.xyz/api/application/users"
